...
const loadTable = async () => {
    ...
    for (i = 0; i < 8; i++) {
        ...
        result[i]['data'].reduce((p, c) => {
            ...
            buttonNode.style.marginTop = `${50 * i + 65}px`;
...
const loadPlayList = async () => {
    play_list = await fetch('DataBase/video.json').then(res => res.json());
    document.getElementById('play_list').innerHTML += `<option value=""> </option>`
    play_list.forEach(x => {
        document.getElementById('play_list').innerHTML += `<option value="${x.name}">${x.name}</option>`
    })
}

const clickPlay = () => {
    let value = document.getElementById('play_list').value;
    document.getElementById('video').innerHTML = play_list.find(x => x.name == value).html;
}
